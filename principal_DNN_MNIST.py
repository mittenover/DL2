...
class DNN(DBN):
    ...
    def retropropagation(self, X, y, nb_epochs, learning_rate, mini_batch_size, verbose=False, step=30):
        self.learning_rate = learning_rate
        self.mini_batch_size = mini_batch_size
        self.nb_epochs = nb_epochs

        if verbose:
            print(f"Entraînement DNN")
        ...
        for epoch in range(nb_epochs):
            ...
            if (epoch % step == 0 or epoch == nb_epochs - 1) and verbose:
                print(f"Epoch {epoch+1}/{nb_epochs}, loss: {np.mean(loss)}")
        return self
